...
const PokemonCacheProvider = props => {
  const reducerHandlers = React.useReducer(pokemonCacheReducer, {})
  return <PokemonCacheContext.Provider {...props} value={reducerHandlers} />
}

const usePokemonCache = () => {
  const contextValue = React.useContext(PokemonCacheContext)
  if (!contextValue) {
